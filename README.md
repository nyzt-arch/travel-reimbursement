...
├── sql.md                            # MySQL 数据库表结构及字典数据初始化脚本
├── README.md                           # 本项目架构及代码阅读指南
├── frontend_development_workflow.md    # 前端开发工作流与步骤指南
...
│       ├── assets/                     # 静态资源与公共样式目录
│       │   └── styles/                 # 样式配置目录
│       │       ├── variables.css       # 全局 CSS 变量定义（主题色、间距等）
│       │       └── main.css            # 全局样式与基础重置样式
│       ├── router/                     # 路由目录
...
└── reimbursement-back/                  # 后端子系统（Spring Boot 4.1 + MyBatis-Plus）
    ...
    └── src/
        ...
        │   │   │   └── GlobalExceptionHandler.java # 全局异常捕获及统一响应处理器
        │   │   ├── config/             # 系统配置层
        ...
        └── test/                       # 单元测试模块
            └── java/com/example/reimbursementback/ReimbursementBackApplicationTests.java # 后端单元测试主类
```
---
...
在报销单详情页 [ReimbursementDetail.vue](file:///d:/Desktop/travel-reimbursement/reimbursement-front/src/views/ReimbursementDetail.vue) 中，通过状态控制子模块的数据联动：
...
当用户点击底部操作栏 [ActionBar.vue](file:///d:/Desktop/travel-reimbursement/reimbursement-front/src/components/detail/ActionBar.vue) 的“确认提交”按钮时，触发以下过程：

1. `ReimbursementDetail.vue` 拦截点击，调用 [utils/validator.ts](file:///d:/Desktop/travel-reimbursement/reimbursement-front/src/utils/validator.ts) 中的 `validateReimbursement(model)` 函数进行强校验。
2. 校验内容包含：
