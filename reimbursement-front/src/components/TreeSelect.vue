...
// Display label of selected node
...
// Flat list or recursive render list to select
const handleSelectNode = (node: BusinessType) => {
  if (node.thereSubordinateNode === '1') {
    // If it has children, we don't select it, we just let the UI handle collapse if we want.
    // For simplicity, leaf selection is enforced.
    return;
  }
